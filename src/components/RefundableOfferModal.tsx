...
export default function RefundableOfferModal({ isOpen, onClose, onAccept }: RefundableOfferModalProps) {
  const checklistItems = [
    '$99 deposit 100% refunded after Owner Update',
    'Book now, pick dates later',
  ];
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} variant="content" maxWidth="max-w-md">
      ...
      <p className="text-gray-500 text-center mb-4 sm:mb-6 px-1 sm:px-2 text-sm sm:text-base">
        Secure your Poconos getaway now. Pick your dates later. No blackout dates through May 2026.
      </p>
      ...
      <button
        onClick={onAccept}
        className="w-full bg-[#F5E531] hover:bg-[#E5D521] text-[#222222] font-bold uppercase py-3 rounded-xl transition-colors text-sm sm:text-base"
      >
        RESERVE NOW
      </button>
