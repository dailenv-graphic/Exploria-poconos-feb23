...
const packageItems = [
  { text: '4-Day/3-Night Luxury Villa Stay', icon: Home },
  { text: 'Locked-in Rate: $99 Refundable Deposit', icon: DollarSign },
  { text: '57% Instant Savings Applied', icon: Percent },
  { text: 'Bonus: 5-Day Cruise or Tubing Passes', icon: Gift },
];
...
export default function PackageInclusionLoader({ onComplete, onClose, isVisible }: PackageInclusionLoaderProps) {
  ...
  return (
    <Modal isOpen={isVisible} onClose={onClose} variant="clean" maxWidth="max-w-md">
          ...
          return (
            ...
            >
              <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
                ...
                {isLoading ? (
                  <Loader2 className="w-4 h-4 text-[#FFFEF0]0 animate-spin" />
                ) : isCompleted ? (
      ...
      <div className="absolute -bottom-3 -left-3 w-12 h-12 bg-gradient-to-br from-[#F5E531] to-[#F5E531] rounded-full opacity-20 blur-xl" />
    </Modal>
  );
}
